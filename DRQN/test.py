...
env.init(client_pool=[("localhost", 10000)], start_minecraft=False,
		 allowDiscreteMovement=["jumpsouth", "move", "turn"],
		 observeRecentCommands=True,
		 videoResolution=False)
# ["move", "turn","jumpsouth;movesouth"]
...
# Results write file
f = open("results_030220.csv", mode="w")
...
def update_policy():
	...
	for r in range(len(policy.episode_rewards[::-1])):
		# try:
		# R = gamma ** r * policy.episode_rewards[::-1][r]
...
def train(episodes):
	...
	for episode in range(episodes):
		...
		for time in range(1000):
			...
			if done:
				total_rewards.append(sum(policy.episode_rewards))
				print(episode, sum(policy.episode_rewards), end='')
				writer.writerow([sum(policy.episode_rewards)])
				break
